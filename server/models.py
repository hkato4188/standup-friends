...
class User(db.Model, SerializerMixin):
    ...
    discussion_responses = db.relationship(
        "DiscussionResponse", backref="submitter")
...
class Group(db.Model, SerializerMixin):
    ...
    def validate_name(self, db_column, name):
        if isinstance(name, str) and name:
            ...
            self.validation_errors.append(
                'Name must be a string and be at least one character long.')
...
class Meeting(db.Model, SerializerMixin):
    ...
    questions = db.relationship(
        "Question", secondary=meeting_question, backref="meetings")
    # relationship: group
    ...
    def validate_topic(self, db_column, topic):
        if isinstance(topic, str) and topic:
            ...
            self.validation_errors.append(
                'Please provide the topic for the meeting.')
...
class Question(db.Model, SerializerMixin):
    ...
    def validate_description(self, db_column, description):
        if isinstance(description, str) and len(description) >= 15:
            ...
            self.validation_errors.append(
                'Please provide a more detailed question description.')
...
class Response(db.Model, SerializerMixin):
    ...
    def validate_content(self, db_column, content):
        if isinstance(content, str) and len(content) >= 300:
            ...
            self.validation_errors.append(
                'Responses must be at least 300 characters long. Please provide more detail and substance to your response.')
...
class DiscussionTopic(db.Model, SerializerMixin):
    ...
    created_at = db.Column(db.DateTime, server_default=db.func.now())
    discussion_responses = db.relationship(
        "DiscussionResponse", backref="discussion_topic")
    ...
    def validate_content(self, db_column, content):
        if isinstance(content, str) and content:
            ...
            self.validation_errors.append(
                'Please briefly describe your discussion topic.')
    ...
    def validate_rating(self, db_column, rating):
        ...
        if isinstance(rating, int) and rating in range(1, 6):
            ...
            self.validation_errors.append(
                'Please select a rating value between 1 - 5.')
...
class DiscussionResponse(db.Model, SerializerMixin):
    ...
    created_at = db.Column(db.DateTime, server_default=db.func.now())
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"))
    discussion_topic_id = db.Column(
        db.Integer, db.ForeignKey("discussion_topics.id"))
    serialize_rules = ("-submitter.discussion_responses",
                       "-discussion_topic.discussion_responses")
#    relationship: submitter
    ...
    def validate_discussion_topic(self, db_column, discussion_topic_id):
        discussion_topic = DiscussionTopic.query.filter(
            DiscussionTopic.id == discussion_topic_id).first()
        if discussion_topic:
